#define MAX_TXD_NAME     (20)
#define MAX_TEXTURE_NAME (32)

enum e_Textures
{
    e_TextureModel,
    e_TextureTXD    [MAX_TXD_NAME char],
    e_TextureName   [MAX_TEXTURE_NAME char]
}

new const g_Textures[][e_Textures] =
{
    {0, !"INVALID", !"INVALID"},
...
new    const g_MaxTextures = sizeof g_Textures;
...
{
    new modelid;
    if(textureid >= 0 && textureid < g_MaxTextures)
        modelid = g_Textures[textureid][e_TextureModel];
    else
        modelid = -1;
    return modelid;
...
stock GetTextureTXD(textureid)
{
    new txd[MAX_TXD_NAME];
    if(textureid >= 0 && textureid < g_MaxTextures)
        strunpack(txd, g_Textures[textureid][e_TextureTXD]);
    else
        strunpack(txd, !"none");
    return txd;
}

stock GetTextureName(textureid)
{
    new name[MAX_TEXTURE_NAME];
    if(textureid >= 0 && textureid < g_MaxTextures)
        strunpack(name, g_Textures[textureid][e_TextureName]);
    else
        strunpack(name, !"none");
    return name;
}
