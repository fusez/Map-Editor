...
public OnFilterScriptInit()
{
    ...
    ClearMap(.objects = false, .vehicles = false, .pickups = true, .attached = true); // Remove pickups & attached objects only
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	if(dialogid == g_NewDialog)
	{
	    ...
	    ClearMap(.objects = true, .vehicles = true, .pickups = true, .attached = true); // Remove every element
